...
The aim of this exercise is to explore these results in greater detail. To do that, it would be good to know the functions of the genes in the Pelobates genome. One important extra file that you will use for this, is therefore an annotation file generated through BLASTing the genes against the [*Xenopus tropicalis* proteome](http://www.ensembl.org/Xenopus_tropicalis/Info/Index) available on Ensembl.  
  
In this exercise we will go through:

1. Comparing sets of genes (venn diagrams and upsets).
2. Pairwise comparisons of fold change (MA plots and volcano plots).
3. Making plots interactive.
4. Make plots ready for publication!
...
## Comparing sets of significant genes
...
This plot tells us that the largest set of genes is shared between the two dorsal-ventral comparisons. After that, the next largest sets are comparison-specific. 
...
The DESeq2 package incorporates a prior on log2 fold changes, resulting in moderated estimates from genes with low counts and highly variable counts, as can be seen by the spread of points on the left side of the MA plot. These have relatively high fold changes, but are not identified as significant.

A useful exercise to reduce this _noise_ of the low-count genes is to apply a "shrinkage" method, which pulls the fold change of these low count, high variability genes closer to zero. The most widely used type is the `apglm` method.
...
As a final tweak, we will take advantage of the `plotly` package, to turn any (or almost any) ggplot object into an interactive, javascript plot! This is literally as easy as:

```{r}
ggplotly(gg_res, tooltip = "text")
```

Notice that the "text" that is shown when hovering over genes can be defined in the aesthetic of the ggplot object.
...
### Making publication-readt plots
